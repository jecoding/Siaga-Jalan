...
<body>
    ...
    <div class="bg-slate-900 border-b border-slate-800 px-4 py-3 flex items-center justify-between z-10 relative">
        ...
        <div class="flex items-center gap-4">
            ...
            <form method="POST" action="{{ route('logout') }}" class="m-0">
                @csrf
                <button type="submit"
                    class="text-xs font-bold text-red-500 hover:text-red-400 bg-red-500/10 hover:bg-red-500/20 px-3 py-1.5 rounded-lg transition">
                    Logout
                </button>
            </form>
        </div>
    ...
    <div id="bottomPanel"
        class="bg-slate-900 border-t border-slate-800 p-4 z-10 relative transition-all duration-500 glow-idle">

        <!-- Warning Banner (hidden by default) -->
        <div id="warningBanner"
            class="hidden mb-3 p-3 bg-red-500/20 border border-red-500/50 rounded-xl text-center danger-pulse">
        ...
        <div id="safeBanner"
            class="hidden mb-3 p-3 bg-emerald-500/10 border border-emerald-500/30 rounded-xl text-center">
            <p class="text-emerald-400 font-bold text-sm">✅ Rute Aman — Selamat Berkendara</p>
        </div>

        <!-- Controls -->
        <div class="flex items-center gap-3">
            <button id="btnTrack" onclick="toggleTracking()"
                class="flex-grow py-3.5 rounded-xl font-bold text-sm transition-all duration-300 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white shadow-lg shadow-blue-600/30">
                <svg id="iconPlay" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd"
                        d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z"
                        clip-rule="evenodd"></path>
                </svg>
                <svg id="iconStop" class="w-5 h-5 hidden" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd"
                        d="M10 18a8 8 0 100-16 8 8 0 000 16zM8 7a1 1 0 00-1 1v4a1 1 0 001 1h4a1 1 0 001-1V8a1 1 0 00-1-1H8z"
                        clip-rule="evenodd"></path>
                </svg>
                <span id="btnLabel">Mulai Pantau</span>git
            </button>
            <button onclick="centerOnMe()"
                class="p-3.5 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 transition shadow-sm"
                title="Pusatkan Peta">
    ...
    <script>
        ...
        async function loadBlackSpots() {
            try {
                ...
                if (!res.ok) throw new Error(`HTTP ${res.status}: Gagal memuat data titik rawan`);

                const data = await res.json();

                // Jika respons adalah error object bukan array
                ...
                data.forEach(spot => {
                    try {
                        if (!spot.geojson) return;

                        // Parse geojson (bisa berupa string atau object)
                        const geometry = typeof spot.geojson === 'string'
                            ? JSON.parse(spot.geojson)
                            : spot.geojson;
                        ...
                        // Skip koordinat yang tidak valid (NaN, Infinite, di luar range)
                        if (isNaN(lat) || isNaN(lng) ||
                            lat < -90 || lat > 90 ||
                            lng < -180 || lng > 180) {
                            console.warn(`Skip spot id=${spot.id} (${spot.name}): koordinat tidak valid [${lng}, ${lat}]`);
                            return;
                        }

                        const radius = spot.radius || 100;

                        // Buffer Zona Bahaya
                        const bufferZone = L.circle([lat, lng], {
                            radius: radius,
                            color: '#ef4444',
                            fillColor: '#ef4444',
                            fillOpacity: 0.25,
                        ...
                        blackSpotsLayerGroup.addLayer(bufferZone);

                        // Marker Pusat (pin merah)
                        const marker = L.marker([lat, lng], { icon: gmapPinIcon })
                            .bindTooltip(`
                                <div style="font-family:Inter,sans-serif;padding:2px 4px">
                                    <b style="color:#dc2626;font-size:10px;text-transform:uppercase;letter-spacing:.05em">⚠ Zona Rawan</b><br/>
                                    <span style="color:#1e293b;font-weight:700">${spot.name}</span><br/>
                                    <span style="color:#64748b;font-size:10px">Radius: ${radius}m</span>
                                </div>
                            `, { direction: 'top', offset: [0, -38], opacity: 0.95 });

                        blackSpotsLayerGroup.addLayer(marker);
                        successCount++;
                    } catch (spotErr) {
                        console.warn(`Skip spot id=${spot.id}: error parsing`, spotErr);
                    }
                });

                console.log(`Black spots loaded: ${successCount}/${data.length} titik berhasil ditampilkan`);
            } catch (e) {
